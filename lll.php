...
if (isset($_GET["id"])) {
    ...
    SELECT product.id, product.price, product.description, product.title, product.datetime_added, 
           product.category_cat_id, product.status_status_id, product.agelimit_age_id,
           GROUP_CONCAT(product_img.img_path) AS images
    FROM product 
            ...
            <div class="container-fluid">
                    ...
                    <div class="col-12 col-lg-6">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="category.php?id=<?php echo $product_data['category_cat_id']; ?>" class="text-light"><?php echo htmlspecialchars($category_name); ?></a></li>
                                <li class="breadcrumb-item active" aria-current="page"><?php echo htmlspecialchars($product_data["title"]); ?></li>
                            </ol>
                        </nav>
                    ...
                    <div class="col-12 feedback-container">
                        ...
                        ?>
                            <div class="feedback">
                                ...
                                <span class="badge bg-<?php echo $feedback_data['type'] == 1 ? 'success' : ($feedback_data['type'] == 2 ? 'warning' : 'danger'); ?>">
                                    <?php echo $feedback_data['type'] == 1 ? 'Positive' : ($feedback_data['type'] == 2 ? 'Neutral' : 'Negative'); ?>
                                </span>
                                <small class="text-muted float-end"><?php echo htmlspecialchars($feedback_data['date']); ?></small>
                            </div>
                        <?php endwhile; ?>
                    </div>
                </div>
                <div class="row mt-4">
                    ...
                    <div class="col-12 d-flex flex-wrap justify-content-center">
                        <?php
                        $related_rs = Database::search("SELECT product.id, product.title, product.price, product_img.img_path FROM product 
                JOIN product_img ON product.id = product_img.product_id WHERE product.id <> '$pid' GROUP BY product.id LIMIT 4");
                        ...
                        ?>
                            <div class="card mx-2 mb-4" style="width: 18rem;">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="card-text">Rs. <?php echo number_format($related_data['price'], 2);
                                    ?></p>
